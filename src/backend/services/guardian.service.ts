...
import type {
  ...
  FamilyMember,
  GuardianPlacement,
  GuardianDashboardPatient,
  CareRequirementMessage,
  WizardAgency,
  CareRequirement,
  GuardianTransaction,
  GuardianInvoice,
  ...
  PastCaregiverReview,
  ReceivedReview,
  PlacementShiftHistory,
  CaregiverTimelineEntry,
  GuardianActivity,
  GuardianDashboardAlert,
...
function normalizeStoredMockPatient(raw: unknown): Patient | null {
  ...
  return {
    ...
    emergencyContactName:
      typeof p.emergencyContactName === "string"
...
function mapCareContractToCareRequirement(c: CareContract): CareRequirement {
  ...
  const cats = Array.isArray(c.meta?.category)
    ? c.meta!.category!.filter(Boolean).map(String)
    : [];
  const careType =
    cats.length > 0
      ? cats.join(", ")
      : c.meta?.title
        ? String(c.meta.title).slice(0, 80)
        : "Care request";
  const sched = c.schedule as
    | { start_date?: string; pattern?: string; shift_type?: string }
    | undefined;
  ...
  const schedule =
    scheduleParts.length > 0 ? scheduleParts.join(" · ") : "Schedule TBD";
  const pr = c.pricing as
    | {
        budget_min?: number;
        budget_max?: number;
  ...
  const pricingModel = pr?.preferred_model || pr?.pricing_model;
  const budget =
    bm != null || bx != null
      ? `\u09F3 ${(bm ?? 0).toLocaleString()} – \u09F3 ${(bx ?? 0).toLocaleString()}${pricingModel === "daily" ? "/day" : pricingModel === "hourly" ? "/hr" : "/mo"}`
      : "Budget TBD";
  const submitted = new Date(c.created_at).toLocaleDateString(undefined, {
    month: "short",
    day: "numeric",
    year: "numeric",
  });
...
export const guardianService = {
    ...
    if (USE_SUPABASE) {
      return sbRead(`gd:placements:${dataCacheScope()}`, async () => {
        ...
        return (data || []).map((d: any) => ({
          ...
          rating: d.rating,
          shiftsCompleted: d.shifts_completed,
          shiftsTotal: d.shifts_total,
        }));
      });
    }
  ...
  async getDashboardPatients(): Promise<GuardianDashboardPatient[]> {
    if (USE_SUPABASE) {
      return sbRead(`gd:dashboard-patients:${dataCacheScope()}`, async () => {
        ...
        return (data || []).map((d: any) => ({
          ...
          statusColor:
            d.status === "stable" ? "var(--cn-purple)" : "var(--cn-green)",
        }));
      });
    }
    return demoOfflineDelayAndPick(
      200,
      [],
      (m) => m.MOCK_GUARDIAN_DASHBOARD_PATIENTS,
    );
  },
  ...
  async getDashboardAlerts(): Promise<GuardianDashboardAlert[]> {
    ...
    return demoOfflineDelayAndPick(
      200,
      [],
      (m) => m.MOCK_GUARDIAN_DASHBOARD_ALERTS,
    );
  },
    ...
    if (USE_SUPABASE) {
      return sbRead(`gd:dashboard-summary:${dataCacheScope()}`, async () => {
        ...
        if (useInAppMockDataset()) {
          ...
          return {
            activePlacements:
              activePlacements || placements.length || base.activePlacements,
  ...
  async getCareRequirementMessages(
    _reqId: string,
  ): Promise<CareRequirementMessage[]> {
    if (USE_SUPABASE) {
      return sbRead(
        `gd:care-req-msgs:${dataCacheScope()}:${_reqId}`,
        async () => {
    ...
    return demoOfflineDelayAndPick(
      200,
      [],
      (m) => m.MOCK_CARE_REQUIREMENT_MESSAGES,
    );
  },
    ...
    if (USE_SUPABASE) {
      return sbRead(`gd:transactions:${dataCacheScope()}`, async () => {
        ...
        const { data, error } = await sbData()
          .from("invoices")
          .select(
            "id, description, issued_date, total, status, from_party_name, to_party_id",
          )
          .eq("to_party_id", userId)
        ...
        return (data || []).map((d: any) => ({
          id: d.id,
          desc:
            d.description || `Payment to ${d.from_party_name || "provider"}`,
          patient: "",
          date: d.issued_date || "",
          amount: d.total || 0,
          type: "debit" as const,
          status:
            d.status === "paid" ? ("completed" as const) : ("pending" as const),
        }));
      });
    }
    return demoOfflineDelayAndPick(
      200,
      [],
      (m) => m.MOCK_GUARDIAN_TRANSACTIONS,
    );
  },
    ...
    if (USE_SUPABASE) {
      return sbRead(`gd:invoices:${dataCacheScope()}`, async () => {
        ...
        const { data, error } = await sbData()
          .from("invoices")
          .select(
            "id, type, description, subtotal, total, status, issued_date, due_date, from_party_name, from_party_role",
          )
          .eq("to_party_id", userId)
          .order("issued_date", { ascending: false });
        if (error) throw error;
        return (data || []).map((d: any) => ({
          ...
          dueDate: d.due_date || "",
          from: {
            name: d.from_party_name || "",
            role: d.from_party_role || "",
          },
        }));
      });
    }
  ...
  async getScheduleAppointments(): Promise<GuardianAppointment[]> {
    if (USE_SUPABASE) {
      return sbRead(
        `gd:schedule-appointments:${dataCacheScope()}`,
        async () => {
          const userId = await currentUserId();
          const { data, error } = await sbData()
            .from("placements")
            .select(
              "id, patient_name, caregiver_name, care_type, schedule, status",
            )
            .eq("guardian_id", userId)
            .eq("status", "active");
          if (error) throw error;
    ...
    return demoOfflineDelayAndPick(
      200,
      [],
      (m) => m.MOCK_GUARDIAN_APPOINTMENTS,
    );
  },
  ...
  async getScheduleTodayEvents(): Promise<GuardianTodayEvent[]> {
    if (USE_SUPABASE) {
      return sbRead(`gd:today-events:${dataCacheScope()}`, async () => {
        ...
        const { data, error } = await sbData()
          .from("shifts")
          .select(
            "id, date, start_time, end_time, status, caregiver_id, patient_id",
          )
          .eq("date", today);
        if (error) throw error;
        ...
        const { data: patients } = await sbData()
          ...
          .in("id", patientIds)
          .eq("guardian_id", userId);
        const guardianPatientIds = new Set(
    ...
    return demoOfflineDelayAndPick(
      200,
      [],
      (m) => m.MOCK_GUARDIAN_TODAY_EVENTS,
    );
  },

  async getReviewsData(): Promise<{
    pastCaregivers: PastCaregiverReview[];
    ...
    if (USE_SUPABASE) {
      return sbRead(`gd:reviews:${dataCacheScope()}`, async () => {
        ...
        const { data: given, error: err1 } = await sb()
  ...
  async getPlacementDetail(
    ...
    shiftHistory: PlacementShiftHistory[];
    caregiverTimeline: CaregiverTimelineEntry[];
  }> {
    if (USE_SUPABASE) {
      return sbRead(
        `gd:placement-detail:${dataCacheScope()}:${_placementId}`,
        async () => {
          const { data: shifts, error } = await sbData()
            .from("shifts")
            .select(
              "id, date, start_time, end_time, status, check_in_time, check_out_time",
            )
  ...
  async getRecentActivity(): Promise<GuardianActivity[]> {
    if (USE_SUPABASE) {
      return sbRead(`gd:recent-activity:${dataCacheScope()}`, async () => {
        ...
        const userId = await currentUserId();
        const { data: recentShifts } = await sbData()
          .from("shifts")
    ...
    return demoOfflineDelayAndPick(
      200,
      [],
      (m) => m.MOCK_GUARDIAN_RECENT_ACTIVITY,
    );
  },
  ...
  async getCaregiverPublicProfile(id: string): Promise<CaregiverPublicProfile> {
    if (USE_SUPABASE) {
      return sbRead(
        `gd:caregiver-profile:${dataCacheScope()}:${id}`,
        async () => {
          const { data, error } = await sbData()
  ...
  async getCaregiverComparison(): Promise<ComparisonCaregiver[]> {
    ...
    return demoOfflineDelayAndPick(
      200,
      [],
      (m) => m.MOCK_COMPARISON_CAREGIVERS,
    );
  },
    ...
    if (USE_SUPABASE) {
      return sbRead(`gd:invoice:${dataCacheScope()}:${id}`, async () => {
        const { data, error } = await sbData()
          ...
          .single();
        if (error) throw error;
        const { data: lineItems } = await sbData()
        ...
        return {
          ...
          toParty: {
            id: d.to_party_id,
            name: d.to_party_name,
            role: d.to_party_role,
          },
  ...
  async reportIncident(data: {
    ...
    if (!useInAppMockDataset()) {
      throw new Error(
        "[CareNet] Connect Supabase or use Demo Access to report incidents.",
      );
    }
    await delay(400);
  ...
  async getOperationalDashboard(): Promise<OperationalDashboardData> {
    return withDashboardFallback(
      ...
      EMPTY_OPERATIONAL_DASHBOARD,
      (m) =>
        mapGuardianOperationalDashboard({
