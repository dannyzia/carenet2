...
export default function AgencyBidManagementPage() {
  ...
  useDocumentTitle(
    tDocTitle("pageTitles.agencyBidManagement", "Agency Bid Management"),
  );
  ...
  const {
    ...
    () => marketplaceService.getMyBids(user?.id ?? ""),
    [user?.id],
  );
  ...
  useEffect(() => {
    ...
    return subscribeToCareContractBids(bidContractIds, user.id, (payload) => {
      ...
      if (
        ...
        const ns = String(
          (payload.newRecord as { status?: string }).status ?? "",
        );
        const os = String(
          (payload.oldRecord as { status?: string }).status ?? "",
        );
        if (ns && ns !== os) {
      ...
      requestAnimationFrame(() => {
        if (expiryLiveRef.current) {
          expiryLiveRef.current.textContent = newlyExpired
            .map((b) =>
              t("agencyBids.bidExpiredAnnounce", { id: b.contract_id }),
            )
            .join(". ");
        }
      });
  ...
  useEffect(() => {
    const unsubscribe = marketplaceService.onPackageSubscription(
      (agencyId, packageTitle) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmDialog
        ...
        title={
          confirmAction?.type === "withdraw"
            ? t("agencyBids.confirmWithdrawTitle")
            : t("agencyBids.confirmAcceptTitle")
        }
        description={
          confirmAction?.type === "withdraw"
            ? t("agencyBids.confirmWithdrawDesc")
            : t("agencyBids.confirmAcceptDesc")
        }
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          {
            label: t("agencyBids.totalBids"),
            value: stats.total,
            color: cn.teal,
            bg: cn.tealBg,
          },
          {
            label: t("agencyBids.pending"),
            value: stats.pending,
            color: cn.amber,
            bg: cn.amberBg,
          },
          {
            label: t("agencyBids.needResponse"),
            value: stats.countered,
            color: cn.purple,
            bg: cn.purpleBg,
          },
          {
            label: t("agencyBids.winRate"),
            value: `${stats.winRate}%`,
            color: cn.green,
            bg: cn.greenBg,
          },
          ...
          <div key={s.label} className="stat-card p-3 text-center">
            <p className="text-lg" style={{ color: s.color }}>
              {s.value}
            </p>
        ...
        <div className="space-y-3">
          {filteredBids.map((bid) => {
            ...
            const isCountered =
              ...
              bid.counter_offer?.from === "patient";
            ...
            return (
              <div
                key={bid.id}
                className={`stat-card p-4 space-y-3 ${bidExpired ? "opacity-60" : ""}`}
              >
                {/* Header */}
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    <p className="text-sm" style={{ color: cn.text }}>
                      {t("agencyBids.bidOnContract", { id: bid.contract_id })}
                    </p>
                    <p className="text-xs" style={{ color: cn.textSecondary }}>
                      {t("agencyBids.submitted", {
                ...
                >
                  <span className="flex items-center gap-1">
                    ...
                    {(bid.proposed_pricing.base_price || 0).toLocaleString()}/
                    {bid.proposed_pricing.pricing_model || "mo"}
                  </span>
                  {bid.proposed_staffing && (
                    <span className="flex items-center gap-1">
                      <Users className="w-3 h-3" />{" "}
                      {t("agencyBids.level", {
                        level: bid.proposed_staffing.required_level,
                      })}
                    </span>
                  )}
                  <span className="flex items-center gap-1">
                    ...
                    {t("agencyBids.score", {
                      score: bid.compliance.overall_score,
                    })}
                  </span>
                    ...
                    >
                      ...
                      {bidExpired
                        ...
                        : expiryInfo.remainingHours <= 24
                          ? t("agencyBids.hLeft", {
                              count: expiryInfo.remainingHours,
                            })
                          : t("agencyBids.dLeft", {
                              count: Math.ceil(expiryInfo.remainingHours / 24),
                            })}
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <div
                      ...
                      style={{
                        ...
                        background:
                          bid.compliance.overall_score >= 80
                            ? cn.green
                            : bid.compliance.overall_score >= 60
                              ? cn.amber
                              : cn.red,
                      }}
                    />
                  ...
                  <span
                    ...
                    style={{ color: cn.textSecondary }}
                  >
                    {t("agencyBids.metCount", {
                      met: bid.compliance.met_count,
                      total: bid.compliance.total_count,
                    })}
                  </span>
                </div>
                  ...
                  >
                    ...
                    {bid.counter_offer.pricing && (
                      <p
                        ...
                        style={{ color: cn.textSecondary }}
                      >
                        {t("agencyBids.proposed")}: ৳
                        {(
                          bid.counter_offer.pricing.base_price || 0
                        ).toLocaleString()}
                        /{bid.counter_offer.pricing.pricing_model || "mo"}
                      </p>
                    )}
                  </div>
                )}
                        ...
                        {respondingTo === bid.id ? (
                          <div className="flex gap-2 items-center">
                            ...
                            <button
                              ...
                              style={{
                                background: "var(--cn-gradient-agency)",
                              }}
                            >
          ...
          {filteredBids.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-sm" style={{ color: cn.textSecondary }}>
                {activeTab === "all"
                  ? t("agencyBids.noBidsYet")
                  : t("agencyBids.noBidsFilter", { status: activeTab })}
              </p>
              <Link
                to="/agency/care-requirement-board"
                className="inline-flex items-center gap-2 px-4 py-2 mt-3 rounded-xl text-white text-sm no-underline"
