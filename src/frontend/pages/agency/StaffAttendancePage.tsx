...
export default function StaffAttendancePage() {
  ...
  const staffList = Array.isArray(attendance) ? [] : (attendance.staff || []);
  const attendanceRows = staffList.map((row) => ({
    name: row.name,
    role: row.role,
    site: "Assigned Shift",
    checkin: row.checkIn || "—",
  ...
  const attendanceDate = Array.isArray(attendance) ? new Date().toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" }) : attendance.date;
