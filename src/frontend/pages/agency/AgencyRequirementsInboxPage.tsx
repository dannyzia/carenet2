...
import { Inbox, Clock, CheckCircle2, Send, XCircle, MessageSquare, AlertCircle, MapPin, Calendar, Heart, ChevronRight, TrendingUp, Timer, Eye } from "lucide-react";
...
interface Requirement { id: string; guardianName: string; guardianVerified: boolean; guardianPlacements: number; patientName: string; patientAge: number; patientCondition: string; careType: string; duration: string; shiftPreference: string; budgetMin: number; budgetMax: number; preferredModel?: string; location: string; specialRequirements: string; submittedDate: string; submittedAgo: string; responseDeadline: string; status: RequirementStatus; priority: "normal" | "urgent" | "low"; isNew: boolean; }
...
const INBOX_STATUSES = ["new", "under-review", "proposal-sent", "accepted", "declined"] as const satisfies readonly RequirementStatus[];

function normalizeRequirementStatus(s: string | undefined | null): RequirementStatus {
  if (s != null && (INBOX_STATUSES as readonly string[]).includes(s)) return s as RequirementStatus;
  return "new";
}
...
export default function AgencyRequirementsInboxPage() {
  ...
  const { data: requirements, loading, error, refetch } = useAsyncData(() => agencyService.getRequirementsInbox());

  if (loading) return <PageSkeleton />;

  if (error) {
    return (
      <div className="cn-card-flat p-8 max-w-lg mx-auto space-y-4 text-center">
        <AlertCircle className="w-12 h-12 mx-auto" style={{ color: "#EF4444" }} aria-hidden />
        <p className="text-sm" style={{ color: cn.text }}>
          {t("pageTitles.requirementsInboxLoadFailed", "Could not load requirements. Check your connection and permissions.")}
        </p>
        <p className="text-xs break-all" style={{ color: cn.textSecondary }}>{error.message}</p>
        <button
          type="button"
  ...
  const list = (requirements ?? []) as Requirement[];

  const filtered = activeTab === "all" ? list : list.filter((r) => normalizeRequirementStatus(r.status) === activeTab);
  const newCount = list.filter((r) => normalizeRequirementStatus(r.status) === "new").length;
  ...
  const responded = list.filter((r) => normalizeRequirementStatus(r.status) !== "new");
  ...
  const avgResponseHours = responseSamples.length > 0
    ...
    : 0;
  const outcomeCount = list.filter((r) => {
    const st = normalizeRequirementStatus(r.status);
    return st === "accepted" || st === "declined";
  }).length;
  const acceptedCount = list.filter((r) => normalizeRequirementStatus(r.status) === "accepted").length;
  ...
  return (
    <>
      <div className="space-y-6">
        ...
        <div className="flex gap-1 overflow-x-auto pb-1 -mx-1 px-1">{tabs.map((tab) => { const isActive = activeTab === tab.key; const count = tab.key === "all" ? list.length : list.filter((r) => normalizeRequirementStatus(r.status) === tab.key).length; return (<button key={tab.key} onClick={() => setActiveTab(tab.key)} className="px-3 py-2 rounded-lg text-sm whitespace-nowrap transition-all flex items-center gap-1.5" style={{ background: isActive ? cn.tealBg : "transparent", color: isActive ? cn.teal : cn.textSecondary }}>{tab.label}<span className="text-xs px-1.5 py-0.5 rounded-full" style={{ background: isActive ? `${cn.teal}20` : `${cn.textSecondary}15` }}>{count}</span></button>); })}</div>
        <div className="space-y-3">{filtered.map((req) => {
          const rowStatus = normalizeRequirementStatus(req.status);
          const statusCfg = statusConfig[rowStatus];
          const StatusIcon = statusCfg.icon;
          const guardianLabel = (req.guardianName ?? "—").trim() || "—";
          const initials = guardianLabel.split(/\s+/).filter(Boolean).map((w) => w[0]).join("").slice(0, 2).toUpperCase() || "—";
          return (
          <div key={req.id} className="finance-card p-4 sm:p-5 transition-all hover:shadow-md" style={{ borderLeft: req.priority === "urgent" ? "3px solid #EF4444" : req.isNew ? "3px solid #0288D1" : undefined }}>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6 mb-3"><div className="flex items-center gap-2"><div className="w-8 h-8 rounded-full flex items-center justify-center text-xs" style={{ background: cn.greenBg, color: cn.green }}>{initials}</div><div><div className="flex items-center gap-1"><span className="text-sm" style={{ color: cn.text }}>{guardianLabel}</span>{req.guardianVerified && <CheckCircle2 className="w-3 h-3" style={{ color: cn.green }} />}</div><span className="text-xs" style={{ color: cn.textSecondary }}>{req.guardianPlacements} previous placement{req.guardianPlacements !== 1 ? "s" : ""}</span></div></div><div className="flex items-center gap-2 text-xs" style={{ color: cn.textSecondary }}><Heart className="w-3 h-3" style={{ color: cn.pink }} /><span>{req.patientName}, {req.patientAge}y</span></div></div>
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 mb-3 text-xs" style={{ color: cn.textSecondary }}><span className="flex items-center gap-1"><Heart className="w-3 h-3" style={{ color: cn.teal }} />{req.careType ?? "—"}</span><span className="flex items-center gap-1"><Clock className="w-3 h-3" />{req.duration ?? "—"}</span><span className="flex items-center gap-1"><Calendar className="w-3 h-3" />{req.shiftPreference ?? "—"}</span><span className="flex items-center gap-1" style={{ color: cn.green }}>{"\u09F3"} {Number(req.budgetMin ?? 0).toLocaleString()} \u2013 {"\u09F3"} {Number(req.budgetMax ?? 0).toLocaleString()}/{req.preferredModel === "daily" ? "day" : "mo"}</span></div>
            ...
            <div className="flex items-center justify-between pt-3" style={{ borderTop: `1px solid ${cn.border}` }}><div className="flex items-center gap-3 text-xs" style={{ color: cn.textSecondary }}><span>Submitted {req.submittedAgo}</span>{rowStatus === "new" && <span className="flex items-center gap-1" style={{ color: "#E8A838" }}><Timer className="w-3 h-3" />{req.responseDeadline}</span>}</div><div className="flex items-center gap-2"><Link to={`/agency/requirement-review/${req.id}`} className="px-3 py-1.5 rounded-lg text-xs text-white flex items-center gap-1" style={{ background: "var(--cn-gradient-agency)" }}>{rowStatus === "new" ? "Review & Respond" : "View Details"}<ChevronRight className="w-3 h-3" /></Link>{(rowStatus === "new" || rowStatus === "under-review") && <Link to={`/agency/messages?requirement=${req.id}`} className="px-3 py-1.5 rounded-lg text-xs border flex items-center gap-1 no-underline cn-touch-target" style={{ borderColor: cn.border, color: cn.textSecondary }}><MessageSquare className="w-3 h-3" /> Message</Link>}</div></div>
