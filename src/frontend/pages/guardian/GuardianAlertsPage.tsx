...
export default function GuardianAlertsPage() {
  ...
  useDocumentTitle(t("pageTitles.guardianAlerts", "Alerts"));
  ...
  return (
    <Section15PageLayout
      title={t("pageTitles.guardianAlerts", "Alerts")}
      subtitle={t("section15.guardianAlertsSubtitle", "Rules evaluated against incoming vitals and logs.")}
    >
      <div className="space-y-3">
        {data.map((r) => (
          <div key={r.id} className="finance-card p-4 flex gap-3 items-start">
            <div className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0" style={{ background: cn.amberBg }}>
              <Bell className="w-4 h-4" style={{ color: cn.amber }} aria-hidden="true" />
            </div>
            <div>
