...
import { CheckCircle, XCircle, AlertCircle, RefreshCw, User, Clock } from "lucide-react";
...
export default function ModeratorActivationsPage() {
  ...
  const loadData = async () => {
    ...
    setError("");
    try {
      const pending = await moderatorService.getPendingActivations();
      setActivations(pending || []);
    } catch (e: any) {
      setError(e.message || i18n.t("activation.moderator.loadError", { ns: "common" }));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleApprove = async (note?: string) => {
    ...
    try {
      await moderatorService.approveActivation(modalState.item.id, "Moderator", note);
      await loadData();
      setModalState({ isOpen: false, type: "approve", item: null });
    } catch (e: any) {
      setError(e.message || i18n.t("activation.moderator.approveError", { ns: "common" }));
    }
  };
  ...
  const handleReject = async (note: string) => {
    if (!modalState.item) return;
    try {
      await moderatorService.rejectActivation(modalState.item.id, "Moderator", note);
      await loadData();
      setModalState({ isOpen: false, type: "reject", item: null });
    } catch (e: any) {
      setError(e.message || i18n.t("activation.moderator.rejectError", { ns: "common" }));
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-900">{i18n.t("activation.moderator.title", { ns: "common" })}</h1>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="bg-white p-4 rounded-lg border border-gray-200">
          ...
          <div className="text-sm text-gray-500">{i18n.t("activation.moderator.allPending", { ns: "common" })}</div>
        </div>
        <div className="bg-white p-4 rounded-lg border border-gray-200">
          <div className="text-2xl font-bold text-blue-600">{stats.caregiver}</div>
          <div className="text-sm text-gray-500">{i18n.t("activation.moderator.caregivers", { ns: "common" })}</div>
        </div>
        <div className="bg-white p-4 rounded-lg border border-gray-200">
          <div className="text-2xl font-bold text-purple-600">{stats.agency}</div>
          <div className="text-sm text-gray-500">{i18n.t("activation.moderator.agencies", { ns: "common" })}</div>
        </div>
        <div className="bg-white p-4 rounded-lg border border-gray-200">
          <div className="text-2xl font-bold text-green-600">{stats.shop}</div>
          <div className="text-sm text-gray-500">{i18n.t("activation.moderator.shops", { ns: "common" })}</div>
        </div>
      </div>
      ...
      <div className="flex space-x-2 mb-6">
        {(["all", "caregiver", "agency", "shop"] as FilterTab[]).map((tab) => (
          ...
          >
            {tab === "all"
              ? i18n.t("activation.moderator.all", { ns: "common" })
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 bg-white rounded-lg border border-gray-200">
          <h3 className="text-lg font-medium text-gray-900 mb-2">{i18n.t("activation.moderator.noPending", { ns: "common" })}</h3>
          <p className="text-gray-500">{i18n.t("activation.moderator.allCaughtUp", { ns: "common" })}</p>
        </div>
      ) : (
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex space-x-3" onClick={(e) => e.stopPropagation()}>
                    ...
                    >
                      ...
                      {i18n.t("activation.moderator.approve", { ns: "common" })}
                    </Button>
                    <Button
                      variant="outline"
                    ...
                    >
                      ...
                      {i18n.t("activation.moderator.reject", { ns: "common" })}
                    </Button>
                  </div>
                </div>
              </li>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-xl shadow-lg max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-start mb-6">
              <h2 className="text-xl font-bold text-gray-900">{i18n.t("activation.moderator.profileDetail", { ns: "common" })}</h2>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium text-gray-700">{i18n.t("activation.moderator.role", { ns: "common" })}</label>
                  <p className="mt-1 capitalize">{selectedItem.role.replace('_', ' ')}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-700">{i18n.t("activation.moderator.registered", { ns: "common" })}</label>
                  <p className="mt-1">{new Date(selectedItem.registeredAt).toLocaleString()}</p>
                </div>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-700">{i18n.t("activation.moderator.auditLog", { ns: "common" })}</label>
                <div className="mt-2 p-4 bg-gray-50 rounded-lg border border-gray-200">
                  <p className="text-sm text-gray-500">{i18n.t("activation.moderator.noHistory", { ns: "common" })}</p>
                </div>
              </div>
            </div>
          </div>
