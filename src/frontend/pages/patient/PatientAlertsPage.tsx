...
export default function PatientAlertsPage() {
  ...
  useDocumentTitle(t("pageTitles.patientAlerts", "Health alerts"));
  ...
  return (
    <Section15PageLayout
      title={t("pageTitles.patientAlerts", "Health alerts")}
      subtitle={t("section15.alertsSubtitle", "Threshold rules for vitals and safety signals.")}
    >
        ...
        {data.length === 0 ? (
          ...
          data.map((r) => (
            <div key={r.id} className="finance-card p-4 flex gap-3 items-start">
              <div className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0" style={{ background: cn.pinkBg }}>
                <Bell className="w-4 h-4" style={{ color: cn.pink }} aria-hidden="true" />
              </div>
              <div>
