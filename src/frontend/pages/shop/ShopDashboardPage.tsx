...
export default function ShopDashboardPage() {
  ...
  const { data: dash, loading, error } = useAsyncData(() => shopService.getOperationalDashboard());
  ...
  if (error || !dash) {
    return (
      <div className="space-y-2 p-6">
        <h1 className="text-xl font-semibold" style={{ color: cn.text }}>{t("dashboard:shop.title")}</h1>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: cn.textSecondary }}>
            {t("dashboard:shop.subtitle")} — {today}
          </p>
        </div>
        {urgent > 0 && (
          <span
      ...
      <ActionBar actions={dash.actions} role="shop" ariaLabelKey="dashboard:shop.opsAriaLabel" />

      <WorkQueue
        title={t("dashboard:shop.workQueueTitle")}
        rows={dash.queue}
        columnLabels={queueColumns}
        emptyLabel={t("dashboard:shop.workQueueEmpty")}
      />
    </div>
  );
}
