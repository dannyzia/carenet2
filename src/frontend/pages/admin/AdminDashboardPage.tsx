...
export default function AdminDashboardPage() {
  ...
  const { data: dash, loading, error } = useAsyncData(() => adminService.getDashboardData());

  if (loading) return <PageSkeleton variant="dashboard" />;
  if (error || !dash) {
    return (
      <div className="space-y-2 p-6">
        <h1 className="text-xl font-semibold" style={{ color: cn.text }}>{t("dashboard:admin.title")}</h1>
  ...
  return (
    <div className="space-y-6">
      ...
      <ActionBar actions={dash.actions} role="admin" ariaLabelKey="dashboard:admin.opsAriaLabel" />

      <WorkQueue
        title={t("dashboard:admin.workQueueTitle")}
        rows={dash.queue}
