...
export default function AuditLogsPage() {
  ...
  const { data, loading } = useAsyncData(() => adminService.getAuditLogs(actionFilter));
  ...
  return (
    <div>
      ...
      <div className="max-w-6xl mx-auto px-6 -mt-16"><div className="finance-card overflow-hidden"><div className="p-8 border-b border-gray-100 flex flex-col md:flex-row justify-between items-center gap-6"><div className="flex items-center gap-6 w-full md:w-auto"><div className="relative flex-1 md:w-80"><Terminal className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300 w-4 h-4" /><input type="text" placeholder="Search logs (UID, Action, IP)..." className="w-full h-11 pl-11 pr-4 rounded-xl border border-gray-100 bg-gray-50/50 text-xs font-mono outline-none focus:bg-white focus:ring-2 focus:ring-gray-200" /></div></div><div className="flex gap-3"><select 
            value={actionFilter} 
            onChange={(e) => setActionFilter(e.target.value)}
            title="Filter by action type"
            className="bg-gray-100 border-none rounded-xl text-[10px] font-black uppercase tracking-widest px-4 h-11 outline-none cursor-pointer"
          ><option value="">All Actions</option><option value="CP_*">Channel Partner (CP_*)</option></select><select className="bg-gray-100 border-none rounded-xl text-[10px] font-black uppercase tracking-widest px-4 h-11 outline-none cursor-pointer" title="Time range"><option>Last 1 Hour</option><option>Last 24 Hours</option><option>Last 7 Days</option></select></div></div>
